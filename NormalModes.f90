...
do j = 1,Nbeads
	do k = 1, Nbeads
			RRtr(1,j) = RRtr(1,j) + C(j,k)*RR(1,k)
			RRtr(2,j) = RRtr(2,j) + C(j,k)*RR(2,k)
			RRtr(3,j) = RRtr(3,j) + C(j,k)*RR(3,k)
			PPtr(1,j) = PPtr(1,j) + C(j,k)*PP(1,k)
			PPtr(2,j) = PPtr(2,j) + C(j,k)*PP(2,k)
			PPtr(3,j) = PPtr(3,j) + C(j,k)*PP(3,k)
	enddo
enddo
...
do j = 1,Nbeads
	do k = 1, Nbeads
		...
		PP(1,j) = PP(1,j) + C(k,j)*PPtr(1,k)
		PP(2,j) = PP(2,j) + C(k,j)*PPtr(2,k)
		PP(3,j) = PP(3,j) + C(k,j)*PPtr(3,k)
	enddo
enddo
...
!------------ Deallocation -----------------------------------------
!---------------------------------------------------------------------
subroutine DestroyNormalModes()
 deallocate(C) 
 deallocate(A11) 
 deallocate(A12) 
 deallocate(A21) 
 deallocate(A22) 
end subroutine DestroyNormalModes
